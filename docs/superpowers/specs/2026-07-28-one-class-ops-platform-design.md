...
嘉壹启航已有全年运营指导手册（`one-class-operations-guide.html`），其中「自用工具」规划包括素材上传、AI 草稿、线索与知识库等。本规格定义 **V1 自用运营网站**：给机构内部使用，把「老师交素材 → 运营出文案/海报 → 登记老带新线索」做成可每天使用的系统。
...
- **机构范围：** 先服务嘉壹启航；数据模型单机构，不强制 `org_id`，架构预留以后扩展。
