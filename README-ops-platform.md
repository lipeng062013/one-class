...
- **TypeScript 与部署**：类型错误会在部署前拦住；服务器 Docker 只跑 `vite build`（避免 VPS 上 `vue-tsc` 超时）。防护链路：
  ...
  3. 服务器 Docker：`npm run build:docker`（仅打包，不再跑 vue-tsc）  
  手动完整构建：`cd frontend && npm run build`；仅类型：`npm run typecheck`
